...
static uint g_dev_idx;

/**********************************************************
 * Interrupt control and handler
 **********************************************************/
static void pn54x_enable_irq(struct pn54x_dev *pn54x_dev)
{
	unsigned long flags;
...
static irqreturn_t pn54x_dev_irq_handler(int irq, void *dev_id)
{
	...
	// wake up the system if enabled
	if (pn54x_dev->wakeup_enabled) {
		pm_wakeup_event(&(pn54x_dev->client->dev), 0);
	}
 ...
 * driver functions
 **********************************************************/
static unsigned int pn54x_dev_poll(
	struct file *filp,
	struct poll_table_struct *wait)
{
	...
	if (!gpio_get_value(pn54x_dev->irq_gpio)) {
		pr_debug("%s : not readble", __func__);
		pn54x_enable_irq(pn54x_dev);
	} else {
		pr_debug("%s : readble", __func__);
		...
		size_t count, loff_t *offset)
{
	...
	#ifdef DEBUG
	print_hex_dump_bytes("pn54x_dev_read : ", DUMP_PREFIX_NONE, tmp, ret);
	#endif /* DEBUG */
		...
		size_t count, loff_t *offset)
{
	...
	#ifdef DEBUG
	print_hex_dump_bytes("pn54x_dev_write: ", DUMP_PREFIX_NONE, tmp, count);
	#endif /* DEBUG */
	/* Write data */
	...
	if (ret > count) {
		pr_err("%s: wrote too many bytes to i2c (%d)\n", __func__, ret);
		return -EIO;
	}
...
static int pn54x_dev_open(struct inode *inode, struct file *filp)
{
	...
	mutex_lock(&pn54x_dev->opened_mutex);
	if (pn54x_dev->opened) {
		ret = -EBUSY;
		goto out;
	}
	...
	pn54x_dev->opened = true;
	pn544_disable(pn54x_dev);

out:
	mutex_unlock(&pn54x_dev->opened_mutex);
	return ret;
}
				...
				unsigned long arg)
{
	...
	pr_info("%s: cmd=0x%08x, arg=%lu\n", __func__, cmd, arg);
	switch (cmd & 0xffff) { // hack to be compatible with cmds that indicate they pass a pointer (but actually pass a value)
	case PN544_SET_PWR:
		if (arg == 2) {
			/* power on w/FW */
...
#endif
{
	...
	ret = devm_gpio_request_one(&client->dev,
								pdata->ven_gpio,
								GPIOF_DIR_IN | (pdata->ven_active_low ? GPIOF_ACTIVE_LOW : 0),
								"nfc_ven");
	if (ret){
	...
	pn54x_dev->ven_gpio = gpio_to_desc(pdata->ven_gpio);
	...
	pn54x_dev->wakeup_enabled = pdata->wakeup_enabled;
	...
	ret = gpiod_direction_output(pn54x_dev->ven_gpio, 0);
	if (ret < 0) {
	...
	mutex_init(&pn54x_dev->opened_mutex);
	pn54x_dev->dev_name = devm_kasprintf(&pn54x_dev->client->dev, GFP_KERNEL,
										 DEVICE_NAME "%u", g_dev_idx);

	/* register as a misc device - character based with one entry point */
	pn54x_dev->pn54x_device.minor = MISC_DYNAMIC_MINOR;
	pn54x_dev->pn54x_device.name = pn54x_dev->dev_name;
	...
	if (pn54x_dev->wakeup_enabled) {
		device_init_wakeup(&(pn54x_dev->client->dev), pn54x_dev->wakeup_enabled);
	}
...
#endif
{
	...
	device_init_wakeup(&(pn54x_dev->client->dev), 0);
	free_irq(client->irq, pn54x_dev);
	misc_deregister(&pn54x_dev->pn54x_device);
	mutex_destroy(&pn54x_dev->read_mutex);
	mutex_destroy(&pn54x_dev->opened_mutex);
...
static SIMPLE_DEV_PM_OPS(pn54x_pm_ops, pn54x_suspend, pn54x_resume);
